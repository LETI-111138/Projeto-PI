...
public class gameinit extends ApplicationAdapter {
    ...
    public void create() {
        ...
        // --- Animações do jogador e inimigos ---
        ...
        // Guardar HP inicial para detetar quando leva dano
    ...
    public void render() {
        ...
        // --- CÂMARA ---
        Mc.getInstance().getPosition().setX(MathUtils.clamp(
                Mc.getInstance().getPosition().getX(),
                0,
                LARGURA_MUNDO - framePlayer.getRegionWidth()));
        Mc.getInstance().getPosition().setY(MathUtils.clamp(
                Mc.getInstance().getPosition().getY(),
                0,
                ALTURA_MUNDO - framePlayer.getRegionHeight()));
        ...
        camera.position.set(
                Mc.getInstance().getPosition().getX() + framePlayer.getRegionWidth() / 2f,
                Mc.getInstance().getPosition().getY() + framePlayer.getRegionHeight() / 2f,
                0);
        ...
        // 2.5 Desenhar Portas (se existirem)
        ...
        // 2.6 Desenhar textos de dano flutuantes
        drawDamageTexts();

        // 3. Desenhar Jogador
        if (framePlayer != null) {
            batch.draw(framePlayer,
                    (int) Mc.getInstance().getPosition().getX(),
                    (int) Mc.getInstance().getPosition().getY());
        }
        ...
        if (gestorEstatico.getTexture("coin_HUD") != null) {
            font.draw(batch, "COINS: " + Mc.getInstance().getBalanceCoins(),
                    Gdx.graphics.getWidth() - 200,
                    Gdx.graphics.getHeight() - 25);
            batch.draw(gestorEstatico.getTexture("coin_HUD"),
    ...
    public static boolean withinbounds(Position position) {
        return position.getX() <= 2000 && position.getX() >= 0 &&
                position.getY() <= 2000 && position.getY() >= 0;
    }
    ...
    public void drawItems() {
        if (gestorEstatico.getTexture("coin") != null && posItems != null) {
            for (staticAssets c : itemObjects) {
                switch (c.getKey()) {
                    case "coin":
                        batch.draw(gestorEstatico.getTexture("coin"),
                                (int) c.getPosition().getX(),
                                (int) c.getPosition().getY());
                        break;
                    case "staticsword":
                        batch.draw(gestorEstatico.getTexture("staticsword"),
                                (int) c.getPosition().getX(),
                                (int) c.getPosition().getY());
                        break;
                }
    ...
    // Criar portas para a sala atual (fixed spots)
    ...
    // Desenhar os textos de dano flutuantes
    ...
    // Classe interna para guardar info dos textos de dano
